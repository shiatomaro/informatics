            ...
            <div class="table-responsive">
                <table class="table table-bordered text-black" id="courseDbTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php
                        $id = 0;
                        $sql = "SELECT * FROM assign_course_tbl";
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="courseName" class="col-form-label"><b>Course Name</b><?php echo $required;?></label>
                    <select class="form-control" id="courseName" name="courseName" required>
                        <option value="" selected disabled>Select Course Name</option>
                        <?php
                        $sql1 = "SELECT * FROM course_tbl WHERE status=1 GROUP BY course_name ASC";
                        $res1 = mysqli_query($db->conn, $sql1);
                        if(mysqli_num_rows($res1) > 0){
                            foreach($res1 as $rs){
                                echo '
                                <option>'.$rs['course_name'].'</option>
                                ';
                            }
                        }else{
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="year_level" name="year_level" required>
                        ...
                        <?php
                        $sql = "SELECT * FROM yearlevel_tbl WHERE status=1 GROUP BY year_level DESC";
                        $res = mysqli_query($db->conn, $sql);
                        if(mysqli_num_rows($res) > 0){
                            foreach($res as $r){
                                echo '
                                <option>'.$r['year_level'].'</option>
                                ';
                            }
                        }else{
                            echo '<option value="" disabled>No year level available!</option>';
                        }
                        ?>
                    </select>
                </div>
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" name="previousYearLevel" id="previousYearLevel">
            </div>
            </form>
        </div>
    </div>
    ...
    $(document).ready(function(){
        ...
        $('.btn-status').on('click', function(){
            ...
            $('#btn-stat').on('click', function(){
                $.ajax({
                    url:"template/assign_courses_action.php",
                    method:"POST",
        ...
        $('#showCourseModal').on('click', function(){
            $('#courseForm')[0].reset();
            $('.modal-title').html("<b>Assign Course</b>");
        ...
        $('.btn-edit').on('click',function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $('#previousYearLevel').val(data.year_level);
                    $('#hiddenid').val(edit_id);
                    $('#action').val('CourseEdit');
                    $('#submitBtn').val('Update');
                    $('#submitBtn').removeClass('btn-success');
                    $('#submitBtn').addClass('btn-primary');
                    $('.modal-title').html('<b>Edit Assign Course</b>');
                    $('#courseModal').modal('show');
                }
            });
        });
