...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary float-left">List of Student Admin/Registrar</h6>
            <?php
            $check = "SELECT * FROM student_administration_tbl";
            $res1 = mysqli_query($db->conn, $check);
            ...
            <div class="table-responsive">
                <table class="table table-bordered text-black" id="courseDbTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php
                        $id = 0;
                        $sql = "SELECT * FROM student_administration_tbl LIMIT 2";
                        ...
                        if(mysqli_num_rows($res) > 0){
                            foreach($res as $row){
                                ...
                                <tr>
                                <td>'.$id.'</td>
                                <td>'.$row['student_administration_name'].'</td>
                                <td>'.$row['position'].'</td>
                                <td>'.$btnStatus.'</td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="courseName" class="col-form-label"><b>Name</b><?php echo $required;?></label>
                    <input type="text" class="form-control" id="courseName" name="courseName" placeholder="Student Admin/Registrar Name" required>
                </div>
                <div class="form-group forPostion">
                    <label for="courseName" class="col-form-label"><b>Position</b><?php echo $required;?></label>
                    <select class="form-control" id="position" name="position" required>
                        <option value="" selected>Select position...</option>
                        <?php
                        $forOp = "SELECT * FROM student_administration_tbl";
                        $forRes = mysqli_query($db->conn, $forOp);
                        if(mysqli_num_rows($forRes) > 0){
                            foreach($forRes as $forPos){
                                if($forPos['position'] == "Student Admin"){
                                    echo '<option value="Registrar">Registrar</option>';
                                }else{
                                    echo '<option value="Student Admin">Student Admin</option>';
                                }
                            }
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" name="action" id="action" value="studentAdminAdd">
    ...
    $(document).ready(function(){
        ...
        $('.btn-status').on('click', function(){
            ...
            $('#btn-stat').on('click', function(){
                $.ajax({
                    url:"template/student_admin_action.php",
                    method:"POST",
        ...
        $('#showCourseModal').on('click', function(){
            $('#courseForm')[0].reset();
            $('.modal-title').html("<b>Add Admin/Registrar</b>");
            $('#action').val("studentAdminAdd");
            $('.forPostion').attr('hidden', false);
            $('#submitBtn').val("Add");
        ...
        $('.btn-edit').on('click',function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $('.modal-title').html('<b>Edit Course</b>');
                    $('#courseModal').modal('show');
                }
            });
        });
        ...
        $('.btn-delete').on('click', function(){
            ...
            $('#modalContentBody').html("Are you sure you want to delete id?");
            $('.modal-title').html("<b>Confirmation</b>");
            $('#courseDeleteModal').modal('show');
